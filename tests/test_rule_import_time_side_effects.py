...
class TestImportTimeSideEffects:
    ...
    def test_module_scope_open_flagged(self, run_rule):
        violations = run_rule(
            import_time_side_effects,
    ...
    def test_function_scope_impure_call_not_flagged(self, run_rule):
        violations = run_rule(
            import_time_side_effects,
            {"pkg/mod.py": 'def f():\n    open("f.txt")\n    print("x")\n'},
        )
    ...
    def test_class_body_impure_call_flagged(self, run_rule):
        violations = run_rule(
            import_time_side_effects,
    ...
    def test_method_body_inside_class_not_flagged(self, run_rule):
    ...
    def test_module_scope_subprocess_run_flagged(self, run_rule):
        violations = run_rule(
            import_time_side_effects,
    ...
    def test_aliased_module_import_flagged(self, run_rule):
        violations = run_rule(
            import_time_side_effects,
    ...
    def test_pure_qualified_call_not_flagged(self, run_rule):
        violations = run_rule(
            import_time_side_effects,
    ...
    def test_module_scope_call_to_impure_project_function_flagged(self, run_rule):
        violations = run_rule(
            import_time_side_effects,
    ...
    def test_module_scope_call_to_pure_project_function_not_flagged(self, run_rule):
        violations = run_rule(
            import_time_side_effects,
    ...
    def test_cross_file_impure_call_flagged(self, run_rule):
        violations = run_rule(
            import_time_side_effects,
            {
    ...
    def test_default_allowlist_passes_logging_getlogger(self, run_rule):
        ...
        assert run_rule(import_time_side_effects, files) == []
        assert run_rule(
            import_time_side_effects,
    ...
    def test_allow_pattern_suppresses_call_by_name(self, run_rule):
        files = {"pkg/mod.py": 'data = open("f.txt")\n'}
        assert len(run_rule(import_time_side_effects, files)) == 1
        assert run_rule(import_time_side_effects, files, {"allow": ["open"]}) == []

    def test_allow_pattern_suppresses_whole_module(self, run_rule):
        files = {
    ...
    def test_allow_pattern_matches_project_symbol_id(self, run_rule):
    ...
    def test_extra_impure_dotted_extends_module_denylist(self, run_rule):
    ...
    def test_extra_impure_bare_extends_builtin_denylist(self, run_rule):
    ...
    def test_extra_impure_also_applies_to_project_function_bodies(self, run_rule):
        # The policy extension flows into the purity analysis of shape 3.
        src = "def setup():\n    log('hi')\n\nsetup()\n"
        files = {"pkg/mod.py": src}
        assert run_rule(import_time_side_effects, files) == []
        violations = run_rule(import_time_side_effects, files, {"extra-impure": ["log"]})
        assert len(violations) == 1
        assert "'pkg.mod:setup'" in violations[0].message

    # -- misc ----------------------------------------------------------------

    def test_one_violation_per_offending_call(self, run_rule):
        violations = run_rule(
            import_time_side_effects,
            {"pkg/mod.py": 'a = open("a")\nb = open("b")\n'},
        )
