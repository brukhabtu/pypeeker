...
def test_engine_runs_require_docstrings(indexed_project):
    ...
    assert any(v.rule == "require-docstrings" and "foo" in v.message for v in violations)
...
def test_check_cli_exits_nonzero_on_violations(tmp_path, monkeypatch, indexed_project):
    """CLI prints violations and exits 1 when any are present."""
...
def test_check_cli_exits_zero_with_no_violations(tmp_path, monkeypatch, indexed_project):
