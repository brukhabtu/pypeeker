"""Move-symbol preconditions (:mod:`pypeeker.refactor.move`, TASK-131)."""
...
def _binder_blind_bindings(root: Node, line_offset: int) -> dict[str, list[_Region]]:
    ...
    nested ``def``/``class``, and (since ``binder/scopes.py`` declares
    ``TYPE_PARAMETER`` symbols) PEP 695 type parameters — and fails for
    exactly two it does not: ``match``/``case`` capture patterns and an
    ...
    genuine use of the same root. Both forms are function-scoped, so the
    region is the nearest enclosing ``def``/``class``/``lambda`` (``root``
    itself when there is none). Only ``identifier`` tokens are decoded,
    which cannot lex non-UTF-8 (architecture.md → "Raw-decode sweep").
    """
...
class MovedBodyClosed(Precondition):
    ...
    **Except in the two binding forms the binder records no symbol for**
    — ``match``/``case`` captures and an unpacking ``as``-target — where a
    locally bound ``a`` does *not* resolve, so "unresolved" alone would
    readmit exactly the spurious carry and spurious refusal the previous
    paragraph rules out. :func:`_binder_blind_bindings` closes that by
    scanning the parsed definition for those two forms and the region each
    is live in; both tiers consult it before treating a bare root as a
    dotted import's. (PEP 695 type parameters used to be a third such form;
    the binder now declares them as ``TYPE_PARAMETER`` symbols, so a
    type-parameter root resolves and never reaches either tier.)
