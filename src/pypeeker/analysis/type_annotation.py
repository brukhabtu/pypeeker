...
The one piece of state this module reads is the ``symbol_id -> Symbol`` map
memoized on each ``FileIndex`` instance by
:func:`pypeeker.analysis.symbols.symbols_by_id` (shared with the call and
write probes), replacing a linear scan per provider call:
``check.rules.prefer_tuple`` asks for one trait per candidate over the same
index, so the scan was O(candidates x symbols). First binding wins on a
duplicate id, exactly reproducing the ``next(...)`` scan the memo replaced —
a last-wins map could change which annotation the trait reports, and with it
a ``prefer-tuple`` finding or an ``InferredListBinding`` refusal.
"""
...
def _type_annotation(file_index: FileIndex, symbol_id: str) -> Trait:
    ...
    """
    symbol = symbols_by_id(file_index).get(symbol_id)
