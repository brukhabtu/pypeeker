"""Application service: ``privatize`` over the DSL engine (TASK-157, A12).

The new-engine twin of the frozen :mod:`pypeeker.app.privatize`. Same
workflow — nominate symbols with the three demotion rules, plan ONE flattened
demotion transaction — but the nomination, the confidence gate and the two
pointwise guards all come off :data:`pypeeker.dsl.DEMOTE` instead of being
restated here and in ``refactor.privatize``'s pre-filter.

Three things this service does *not* do, each deliberately:
...
* **No message parsing.** ``check/demotion.py`` recovers each candidate's
  symbol id by running three regexes over the finding's *message text*, which
  makes rule wording a load-bearing contract. A DSL row knows its own anchor.
* **No second heuristic filter.** ``skip_heuristic`` is
...
Temporary name: renamed onto ``app/privatize.py`` at B4, along with
``run_dsl_privatize`` -> ``run_privatize``.
"""
...
from pypeeker.refactor import PrivatizeOutcome, SkippedSymbol, plan_privatize_intents
from pypeeker.storage import IndexStore, TransactionStore

__all__ = ["PrivatizeReport", "run_dsl_privatize"]
...
class PrivatizeReport:
    """The result of :func:`run_dsl_privatize`.
...
def _prefilter_positions(
    ...
    ``plan_privatize_intents`` reports its skips in submission order but
...
def run_dsl_privatize(
    ...
    :func:`~pypeeker.refactor.plan_privatize_intents`.
