"""Application service: the ``check`` command's run, on the **new** engine.

The DSL twin of :mod:`pypeeker.app.check_run`, built beside it while both
engines exist. Everything ``check`` does between parsing its flags and
printing — read ``[tool.pypeeker]``, refuse an import-boundaries table that
would enforce nothing, import the configured plugin modules, resolve the rule
names, seed the born-private ratchet, run the rules, sort — lives here so the
CLI keeps only options, output and exit codes.

**Temporary name.** This module is ``check_run2`` only for the duration of the
flip: phase A builds it beside the frozen ``check_run`` (which the CLI still
calls) and phase B deletes that one and renames this to ``check_run.py``, with
``DslCheckRun`` → ``CheckRun``, ``run_dsl_check`` → ``run_check``,
``update_dsl_baseline`` → ``update_check_baseline`` and ``dsl_baseline_delta``
→ ``check_baseline_delta``. Nothing outside the barrel and this segment's
tests should learn the temporary spellings.

Three things differ from the frozen service, each deliberate:

- **An unknown configured rule name refuses** (``UnknownExpressionError``,
  naming every known id) where ``CheckEngine.run`` silently skips it — a
  typo'd rule id used to read as a clean run.
- **The run record carries the config, not an engine.** ``check --fix``'s
  fixpoint needs to re-run the rules against a simulated store, which on the
  new engine means a fresh :class:`~pypeeker.dsl.Corpus` over that store —
  so :class:`DslCheckRun` hands on ``src``/``rules``/``options`` and builds
  the corpus at the point of use.
- **Findings are sorted explicitly.** ``Violation`` is ``order=True`` and the
  frozen engine sorts; ``Finding`` is not orderable and the DSL returns rows
  in per-rule index order.
"""
...
# ``tests/test_app_check_run2.py`` asserts the spelling resolves.
...
class CheckConfigError(Exception):
    ...
    The new engine's twin of ``check.engine.CheckConfigError``, carrying the
    same message text: the two never coexist in one call path (the CLI reaches
    exactly one run service), and the frozen one dies with ``check/``.
    """
...
def run_dsl_check(
    ...
    return DslCheckRun(findings=findings, src=src, rules=rules, options=options)


@dataclass(frozen=True)
class DslBaselineUpdate:
...
def update_dsl_baseline(root: Path, findings: list[Finding]) -> DslBaselineUpdate:
    ...
    return DslBaselineUpdate(recorded=sum(counts.values()), path=path)


def dsl_baseline_delta(root: Path, findings: list[Finding]) -> DslBaselineDelta:
